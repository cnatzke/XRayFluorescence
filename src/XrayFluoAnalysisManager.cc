...
void XrayFluoAnalysisManager::book()
{
  ...
  if (phaseSpaceFlag) 
    ...
    // Create primary NTuple
    man->CreateNtuple("101","Primary Particle");
    fNtColID[0] = man->CreateNtupleDColumn("inputEnergy");
    man->FinishNtuple();
    G4cout << "Created primary particle ntuple" << G4endl;
        
    // Book output Tuple (ID = 1)
    man->CreateNtuple("102","OutputNTuple");
    fNtColID[1] = man->CreateNtupleIColumn("particleID");      
    fNtColID[2] = man->CreateNtupleDColumn("outputEnergy");
    fNtColID[3] = man->CreateNtupleDColumn("momentumTheta");
    fNtColID[4] = man->CreateNtupleDColumn("momentumPhi");
    fNtColID[5] = man->CreateNtupleIColumn("processes");
    fNtColID[6] = man->CreateNtupleIColumn("material");
    fNtColID[7] = man->CreateNtupleIColumn("origin");
    fNtColID[8] = man->CreateNtupleDColumn("depth");
    man->FinishNtuple();
...
void XrayFluoAnalysisManager::LoadGunData(G4String fileName, G4bool raileighFlag) 
{
  ...
  analysisReader->SetNtupleDColumn("inputEnergy", energy);
...
void XrayFluoAnalysisManager::analyseStepping(const G4Step* aStep)
{
  ...
  if (phaseSpaceFlag){       
	...
	{	
	  ...
	  //Fill ntuple	 
	  man->FillNtupleIColumn(2,fNtColID[1],part);
	  man->FillNtupleDColumn(2,fNtColID[2],particleEnergy/keV);
	  man->FillNtupleDColumn(2,fNtColID[3],momentum.theta());
	  man->FillNtupleDColumn(2,fNtColID[4],momentum.phi());
	  man->FillNtupleIColumn(2,fNtColID[5],parent); 
	  man->FillNtupleIColumn(2,fNtColID[6],sampleMat);
	  man->FillNtupleIColumn(2,fNtColID[7],isBornInTheSample);
	  man->FillNtupleDColumn(2,fNtColID[8],particleDepth);
	  man->AddNtupleRow(2);
...
void XrayFluoAnalysisManager::analysePrimaryGenerator(G4double energy)
{
  ...
  G4AnalysisManager* man = G4AnalysisManager::Instance();
  if (phaseSpaceFlag){
	  man->FillNtupleDColumn(1,fNtColID[0],energy/keV);
	  man->AddNtupleRow(1);
  }
